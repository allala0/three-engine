...
class t{
    constructor(i,e,s,n,r='div'){
        ...
        const i=this.parent.add(this.object,this.property,t);return i.name(this._name),this.destroy(),i;
    }min(t){
        return this;
    }max(t){
        return this;
    ...
    let i,e;return(i=t.match(/(#|0x)?([a-f0-9]{6})/i))?e=i[2]:(i=t.match(/rgb\(\s*(\d*)\s*,\s*(\d*)\s*,\s*(\d*)\s*\)/))?e=parseInt(i[1]).toString(16).padStart(2,0)+parseInt(i[2]).toString(16).padStart(2,0)+parseInt(i[3]).toString(16).padStart(2,0):(i=t.match(/^#?([a-f0-9])([a-f0-9])([a-f0-9])$/i))&&(e=i[1]+i[1]+i[2]+i[2]+i[3]+i[3]),!!e&&'#'+e;
}const s={isPrimitive:!0,match:t=>'string'==typeof t,fromHexString:e,toHexString:e},n={isPrimitive:!0,match:t=>'number'==typeof t,fromHexString:t=>parseInt(t.substring(1),16),toHexString:t=>'#'+t.toString(16).padStart(6,0)},r={isPrimitive:!1,match:Array.isArray,fromHexString(t,i,e=1){
        const s=n.fromHexString(t);i[0]=(s>>16&255)/255*e,i[1]=(s>>8&255)/255*e,i[2]=(255&s)/255*e;
    },toHexString:([t,i,e],s=1)=>n.toHexString(t*(s=255/s)<<16^i*s<<8^e*s<<0)},l={isPrimitive:!1,match:t=>Object(t)===t,fromHexString(t,i,e=1){
        const s=n.fromHexString(t);i.r=(s>>16&255)/255*e,i.g=(s>>8&255)/255*e,i.b=(255&s)/255*e;
    },toHexString:({r:t,g:i,b:e},s=1)=>n.toHexString(t*(s=255/s)<<16^i*s<<8^e*s<<0)},o=[s,n,r,l];class a extends t{
    constructor(t,i,s,n){
        var r;super(t,i,s,'color'),this.$input=document.createElement('input'),this.$input.setAttribute('type','color'),this.$input.setAttribute('tabindex',-1),this.$input.setAttribute('aria-labelledby',this.$name.id),this.$text=document.createElement('input'),this.$text.setAttribute('type','text'),this.$text.setAttribute('spellcheck','false'),this.$text.setAttribute('aria-labelledby',this.$name.id),this.$display=document.createElement('div'),this.$display.classList.add('display'),this.$display.appendChild(this.$input),this.$widget.appendChild(this.$display),this.$widget.appendChild(this.$text),this._format=(r=this.initialValue,o.find(t=>t.match(r))),this._rgbScale=n,this._initialValueHexString=this.save(),this._textFocused=!1,this.$input.addEventListener('input',()=>{
            this._setValueFromHexString(this.$input.value);
        }),this.$input.addEventListener('blur',()=>{
            this._callOnFinishChange();
        }),this.$text.addEventListener('input',()=>{
            const t=e(this.$text.value);t&&this._setValueFromHexString(t);
        ...
        if(this._format.isPrimitive){
            const i=this._format.fromHexString(t);this.setValue(i);
        }
    ...
    constructor(t,i,e,s,n,r){
        super(t,i,e,'number'),this._initInput(),this.min(s),this.max(n);const l=void 0!==r;this.step(l?r:this._getImplicitStep(),l),this.updateDisplay();
    }min(t){
        ...
        const t=this.getValue();if(this._hasSlider){
            let i=(t-this._min)/(this._max-this._min);i=Math.max(0,Math.min(i,1)),this.$fill.style.width=100*i+'%';
        }return this._inputFocused||(this.$input.value=t),this;
    }_initInput(){
        this.$input=document.createElement('input'),this.$input.setAttribute('type','number'),this.$input.setAttribute('step','any'),this.$input.setAttribute('aria-labelledby',this.$name.id),this.$widget.appendChild(this.$input),this.$disable=this.$input;const t=t=>{
            const i=parseFloat(this.$input.value);isNaN(i)||(this._snapClampSetValue(i+t),this.$input.value=this.getValue());
        };let i,e,s,n,r,l=!1;const o=t=>{
                if(l){
                    const s=t.clientX-i,n=t.clientY-e;Math.abs(n)>5?(t.preventDefault(),this.$input.blur(),l=!1,this._setDraggingStyle(!0,'vertical')):Math.abs(s)>5&&a();
                }if(!l){
                    const i=t.clientY-s;r-=i*this._step*this._arrowKeyMultiplier(t),n+r>this._max?r=this._max-n:n+r<this._min&&(r=this._min-n),this._snapClampSetValue(n+r);
                }s=t.clientY;
            },a=()=>{
            ...
            const t=parseFloat(this.$input.value);isNaN(t)||this.setValue(this._clamp(t));
        ...
        this._hasSlider=!0,this.$slider=document.createElement('div'),this.$slider.classList.add('slider'),this.$fill=document.createElement('div'),this.$fill.classList.add('fill'),this.$slider.appendChild(this.$fill),this.$widget.insertBefore(this.$slider,this.$input),this.domElement.classList.add('hasSlider');const t=t=>{
                const i=this.$slider.getBoundingClientRect();let e=(s=t,n=i.left,r=i.right,l=this._min,o=this._max,(s-n)/(r-n)*(o-l)+l);var s,n,r,l,o;this._snapClampSetValue(e);
            },i=i=>{
                t(i.clientX);
            },e=()=>{
            ...
            };let s,n,r=!1;const l=i=>{
                i.preventDefault(),this._setDraggingStyle(!0),t(i.touches[0].clientX),r=!1;
            },o=i=>{
                if(r){
                    const t=i.touches[0].clientX-s,e=i.touches[0].clientY-n;Math.abs(t)>Math.abs(e)?l(i):(window.removeEventListener('touchmove',o),window.removeEventListener('touchend',a));
                }
            ...
            },h=this._callOnFinishChange.bind(this);let d;this.$slider.addEventListener('mousedown',s=>{
            ...
            if(Math.abs(t.deltaX)<Math.abs(t.deltaY)&&this._hasScrollBar)return;t.preventDefault();const i=this._normalizeMouseWheel(t)*this._step;this._snapClampSetValue(this.getValue()+i),this.$input.value=this.getValue(),clearTimeout(d),d=setTimeout(h,400);
        });
        ...
        let{deltaX:i,deltaY:e}=t;Math.floor(t.deltaY)!==t.deltaY&&t.wheelDelta&&(i=0,e=-t.wheelDelta/120,e*=this._stepExplicit?1:10);return i+-e;
    }_arrowKeyMultiplier(t){
        let i=this._stepExplicit?1:10;return t.shiftKey?i*=10:t.altKey&&(i/=10),i;
    }_snap(t){
        const i=Math.round(t/this._step)*this._step;return parseFloat(i.toPrecision(15));
    }_clamp(t){
        return t<this._min&&(t=this._min),t>this._max&&(t=this._max),t;
    }_snapClampSetValue(t){
        this.setValue(this._clamp(this._snap(t)));
    }get _hasScrollBar(){
        const t=this.parent.root.$children;return t.scrollHeight>t.clientHeight;
    }get _hasMin(){
    ...
    constructor(t,i,e,s){
        super(t,i,e,'option'),this.$select=document.createElement('select'),this.$select.setAttribute('aria-labelledby',this.$name.id),this.$display=document.createElement('div'),this.$display.classList.add('display'),this._values=Array.isArray(s)?s:Object.values(s),this._names=Array.isArray(s)?s:Object.keys(s),this._names.forEach(t=>{
            const i=document.createElement('option');i.innerHTML=t,this.$select.appendChild(i);
        ...
        const t=this.getValue(),i=this._values.indexOf(t);return this.$select.selectedIndex=i,this.$display.innerHTML=-1===i?t:this._names[i],this;
    }
}class u extends t{
    constructor(t,i,e){
    ...
    constructor({parent:t,autoPlace:i=void 0===t,container:e,width:s,title:n='Controls',injectStyles:r=!0,touchStyles:l=!0}={}){
        ...
        }),this.$title.addEventListener('touchstart',()=>{}),this.$children=document.createElement('div'),this.$children.classList.add('children'),this.domElement.appendChild(this.$title),this.domElement.appendChild(this.$children),this.title(n),l&&this.domElement.classList.add('allow-touch-styles'),this.parent)return this.parent.children.push(this),this.parent.folders.push(this),void this.parent.$children.appendChild(this.domElement);this.domElement.classList.add('root'),!p&&r&&(!function(t){
            const i=document.createElement('style');i.innerHTML=t;const e=document.querySelector('head link[rel=stylesheet], head style');e?document.head.insertBefore(i,e):document.head.appendChild(i);
        ...
        if(Object(s)===s)return new c(this,t,e,s);const l=t[e];switch(typeof l){
        case'number':return new d(this,t,e,s,n,r);case'boolean':return new i(this,t,e);case'string':return new u(this,t,e);case'function':return new h(this,t,e);
        ...
        const i={controllers:{},folders:{}};return this.controllers.forEach(t=>{
            if(!(t instanceof h)){
                if(t._name in i.controllers)throw new Error(`Cannot save GUI with duplicate property "${t._name}"`);i.controllers[t._name]=t.save();
            }
        }),t&&this.folders.forEach(t=>{
            if(t._title in i.folders)throw new Error(`Cannot save GUI with duplicate folder "${t._title}"`);i.folders[t._title]=t.save();
        }),i;
    }open(t=!0){
        ...
        return this._closed=!t,this.$title.setAttribute('aria-expanded',!this._closed),requestAnimationFrame(()=>{
            const i=this.$children.clientHeight;this.$children.style.height=i+'px',this.domElement.classList.add('transition');const e=t=>{
            ...
            };this.$children.addEventListener('transitionend',e);const s=t?this.$children.scrollHeight:0;this.domElement.classList.toggle('closed',!t),requestAnimationFrame(()=>{
        ...
        let t=Array.from(this.controllers);return this.folders.forEach(i=>{
            t=t.concat(i.controllersRecursive());
        }),t;
    }foldersRecursive(){
        let t=Array.from(this.folders);return this.folders.forEach(i=>{
